...
test('Watch live duration targets and Speed up ratio are enforced', () => withApp((app, window, ts2) => {
  ...
  assert.ok(oneWatch >= 20000 && oneWatch <= 24000, `one-leg Watch live target failed: ${oneWatch}`);
  assert.ok(fourWatch >= 25000 && fourWatch <= 32000, `four-leg Watch live target failed: ${fourWatch}`);
  assert.ok(oneFast >= 7000 && oneFast <= 9000, `one-leg Speed up target failed: ${oneFast}`);
