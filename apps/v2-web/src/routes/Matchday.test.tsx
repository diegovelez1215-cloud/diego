...
describe('V2 Matchday states', () => {
  it('keeps loading non-blank and layout-stable', async () => {
    ...
    expect(app.querySelector('.v2-fixture-stage')).toBeTruthy();
  ...
  it('keeps request errors useful and distinct from unavailable', async () => {
    ...
    expect(app.querySelector('.v2-fixture-stage')).toBeTruthy();
    const stage = app.querySelector('.v2-fixture-stage');
  ...
  it('uses live treatment only for a validated live fixture', async () => {
    ...
    expect(app.querySelector('.v2-fixture-stage[data-status="live"]')).toBeTruthy();
    expect(app.querySelector('.v2-fixture-stage .v2-status-mark--live')?.textContent).toContain('63');
    expect([...app.querySelectorAll('.v2-fixture-stage__team > b')].map((node) => node.textContent)).toEqual(['1', '0']);
  });
  ...
  it('truth-gates a validated live status whose score is unresolved', async () => {
    ...
    expect(app.querySelector('.v2-fixture-stage[data-status="live"]')).toBeTruthy();
    expect([...app.querySelectorAll('.v2-fixture-stage__team > b')].map((node) => node.textContent)).toEqual(['–', '–']);
    expect(app.querySelector('.v2-fixture-stage')?.textContent).not.toContain('1–0');
  });
});
