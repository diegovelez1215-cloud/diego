...
export function buildPresentationFrames(plan: MatchPlan, input: MatchInput): readonly MatchFrame[] {
  ...
  let priorScore = { home: 0, away: 0 };
  let priorPossession = plan.events[0]?.teamId ?? input.home.id;
  plan.events.forEach((event, eventIndex) => {
    const travels = event.action.type === 'pass' || event.action.type === 'shot';
    const resolvesShot = event.action.type === 'goal' || event.action.type === 'save';
    const steps = travels ? 7 : resolvesShot ? 3 : event.action.type === 'pivotal-entry' ? 3 : 4;
    const possessionResolves = event.action.type === 'turnover' || event.action.type === 'tackle' || event.action.type === 'goal' || event.action.type === 'save';
    for (let step = 0; step < steps; step++) {
      const progress = step / (steps - 1);
      const actor = event.action.actor ?? null;
      const players = statePlayers(input, event.teamId, actor, progress, event.action);
      const ball = travels || resolvesShot
        ? { x: lerp(event.start.x, event.end.x, progress), y: lerp(event.start.y, event.end.y, progress) }
        : (actor ? players.find((player) => player.id === actor)?.position ?? event.start : event.start);
      const applyScore = event.action.type === 'goal' && progress < 1 ? priorScore : event.scoreAfter;
      const possessionTeamId = possessionResolves && progress < 1 ? priorPossession : event.possessionAfter;
      const ballOwnerId = event.action.type === 'pass'
        ? (progress < 1 ? actor : event.action.target ?? null)
        : event.action.type === 'save'
          ? (progress < 1 ? null : event.action.keeper ?? null)
          : event.action.type === 'shot' || event.action.type === 'goal'
            ? (progress < 1 && event.action.type === 'shot' ? actor : null)
            : actor;
      frames.push(Object.freeze({
        ...
        score: Object.freeze(applyScore),
        possessionTeamId,
        ball: Object.freeze(ball),
        ballOwnerId,
        players: Object.freeze(players),
        action: event.action,
        momentumContext: commentary(event, input.home, input.away),
        meaningful: step === 0,
      }));
    }
    priorScore = event.scoreAfter;
    priorPossession = event.possessionAfter;
  });
